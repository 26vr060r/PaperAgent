...
3. 各論文について以下を整理する。
   ...
   - 著者
   - 公開日
   - **発表元（査読付き）**: 受理された国際学会・国内学会・ジャーナル名（例: CVPR 2024, AAAI 2025, NeurIPS 2023, ACL 2024, IEEE Transactions on ...）。査読付きで受理された会議・学会・ジャーナルが分かる表記にする。arXiv のみで未掲載の場合は「査読付き未確認（arXiv プレプリント）」と記載する。
...
**著者**: ...
**公開日**: ...
**発表元（査読付き）**: CVPR 2024 / AAAI 2025 / NeurIPS 2023 など
...
- **Google Scholar**: 論文詳細の掲載誌名・会議名を確認する。
- **arXiv**: `journal_ref` や `comments`（例: "Accepted to CVPR 2024"）を確認する。
- **Semantic Scholar / Connected Papers**: `venue` 情報を確認する。
- **論文 PDF**: 1 ページ目のフッター・ヘッダーに記載される会議名・ジャーナル名を確認する。
- 複数ソースで情報が異なる場合は、査読付きで正式受理された名称を優先する。
